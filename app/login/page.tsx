...
const LoginPage = () => {
  ...
  const handleSubmit = async (event) => {
    try {
      ...
      setLoading(true);
      let res: any = await signinProfessional({
        email,
        password,
        userType: "client",
      });
      ...
      if (res?.status !== 400 || res?.response?.data?.status === "success") {
        if (res?.isTwoFactAuth === false || !res?.isTwoFactAuth) {
          ...
          let ver: any = await VerifyUser(
            res?.response?.data?.token,
            res?.response?.data?.userType
          );
          ...
          setTempUserData({
            ...tempUserData,
            userId:
              res.response?.data?.data[0]?.userId || Cookies.get("userId"),
            userEmail:
              res.response?.data?.data[0]?.userEmail ||
              Cookies.get("userEmail"),
            userType:
              res.response?.data?.data[0]?.userType || Cookies.get("userType"),
          });
          // redirect
