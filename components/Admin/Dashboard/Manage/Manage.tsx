...
export default function Manage() {
  ...
  useEffect(() => {
    const item =
      manageData && manageData?.find((data) => data?.pageName == "pages");
    setSelectedItems(item);
  }, []);

  // BACKEND INTEGRATION
  const { verifyAdmin }: any = useUserContext();
          ...
          <div className="w-full 2xl:container 2xl:mx-auto h-auto lg:h-screen overflow-hidden flex-col lg:flex-row flex">
            ...
            <div className="w-auto flex-grow px-3 py-2 overflow-y-scroll relative bg-slate-100 hiddenScroll">
              ...
              <div className="flex justify-between items-start">
                ...
                <button
                  className="text-[18px] border ps-3 pe-24 shadow bg-white block rounded-md py-1"
              ...
              <div className="bg-white w-full h-full rounded-md shadow border border-black/40 mt-8">
                <div className="flex w-full max-sm:h-[80vh] h-full">
                  ...
                  <div className="w-[250px] h-full overflow-y-scroll hiddenScroll overflow-x-hidden p-4 border-e border-black/40">
                    <ul>
                      {manageData &&
                        manageData?.map((data: any, i: number) => (
                          <li key={i} className="pb-3">
                            <button
                              onClick={() => setSelectedItems(data)}
                              className={`text-[#07242B] text-[15px] sm:text-[17px] font-semibold py-[8px] px-4 rounded-md ${data?.pageName == selectedItems?.pageName ? "bg-[#E6E6E6]" : "bg-transparent"} uppercase`}
                            >
                              {data?.pageName}
                            </button>
                          </li>
                  ...
                  <div className="flex flex-grow w-auto h-full overflow-y-scroll hiddenScroll overflow-x-hidden">
                    {selectedItems && selectedItems?.pageName == "pages" && (
                      <PagesPage data={selectedItems} />
                    )}
                    {selectedItems &&
                      selectedItems?.pageName == "categories" && (
                        <CategoriesPage data={selectedItems} />
                      )}
                    {selectedItems && selectedItems?.pageName == "services" && (
                      <ServicesPage data={selectedItems} />
                    )}
                    {selectedItems && selectedItems?.pageName == "wallet" && (
                      <WalletPage data={selectedItems} />
                    )}
                    {selectedItems && selectedItems?.pageName == "points" && (
                      <PointsPage data={selectedItems} />
                    )}
                    {selectedItems && selectedItems?.pageName == "badges" && (
                      <BadgesPage data={selectedItems} />
                    )}
                    {selectedItems && selectedItems?.pageName == "ranking" && (
                      <RankingPage data={selectedItems} />
                    )}
                    {selectedItems &&
                      selectedItems?.pageName == "job dialogue" && (
                        <JobDialogue data={selectedItems} />
                      )}
                    {selectedItems &&
                      selectedItems?.pageName == "job assign" && (
                        <JobAssign data={selectedItems} />
                      )}
                  </div>
                </div>
              </div>
