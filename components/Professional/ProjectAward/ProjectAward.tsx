                            ...
                            projectsData?.map((data: any, i: number) => (
                                <div key={i} className="bg-white shadow-md border border-[#07242B66] p-8 w-full rounded-md mb-3 relative overflow-hidden">
                                    ...
                                    <div className="flex gap-5 flex-col md:flex-row justify-center pt-5">
                                        <Link className="flex items-center justify-center font-bold px-5 text-[15px] py-4 text-black gap-1 bg-white border-2 border-[#FFBE00]" href={`/professional/my-responses/details/${data?.projectId}`}>
                                            View Project Details
                                            <IoArrowForward className="text-black size-[20px]" />
                                        </Link>
                                        <Link className="flex items-center justify-center font-bold px-5 text-[15px] py-4 text-black gap-1 bg-[#FFBE00]" href={`/professional/my-responses/proposal/${data?.projectId}`}>
                                            View Proposals
                                            <IoArrowForward className="text-black size-[20px]" />
                                        </Link>
                                        <Link className="flex items-center justify-center font-bold px-5 text-[15px] py-4 text-white gap-1 bg-[#07242B]" href={`/professional/chat`}>
                                            Chat Client
