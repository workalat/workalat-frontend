...
const SideNav = ({ isClientDashboard, setIsClientDashboard }: SideNavProps) => {
  ...
  return (
    <Box className="w-full md:w-80 bg-main text-white h-full md:h-max p-6 flex flex-col lg:ml-3 md:rounded-lg">
      {
        isClientDashboard ? <Box className="flex items-center mb-5 mt-2">
          ...
          <Box className="ml-4 flex-grow">
            <Typography className="font-semibold text-lg flex gap-2 items-center">Anita Maika <span className="border border-white rounded-full h-7 w-7 flex items-center justify-center"><FaUserEdit /></span></Typography>
      ...
      <Box className="flex-grow space-y-4">
        ...
        {[
          ...
          { icon: <ConfirmationNumberIcon />, text: "Support Tickets", routePath: `${isClientDashboard ? "client/dashboard/support-tickets" : "professional/dashboard/support-tickets"}` },
